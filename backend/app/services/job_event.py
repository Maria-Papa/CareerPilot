...
class JobEventService(BaseService[JobEvent]):
    ...
    def list_for_job(self, session: Session, job_id: int, *, offset: int = 0, limit: int = 100) -> Sequence[JobEvent]:
    ...
    def create_for_job(self, session: Session, job_id: int, data: JobEventCreate) -> JobEvent:
    ...
    def add_payload_data(self, session: Session, event: JobEvent, extra_payload: dict) -> JobEvent:
