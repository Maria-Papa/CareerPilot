...
class JobRepository(SoftDeleteBaseRepository[Job]):
    ...
    def list_for_user(self, session: Session, user_id: int, *, offset: int = 0, limit: int = 100) -> Sequence[Job]:
        stmt = self._base_query().where(Job.user_id == user_id).offset(offset).limit(limit)
        return session.execute(stmt).scalars().all()

    def add_status_history(self, session: Session, history: JobStatusHistory) -> JobStatusHistory:
        session.add(history)
        session.commit()
        session.refresh(history)
        return history

    def add_file_attachment(self, session: Session, attachment: JobFileAttachment) -> JobFileAttachment:
